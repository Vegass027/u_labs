...
export async function createWithdrawalRequest(
  ...
  logger.info({ withdrawalId: data.id, amount: input.amount }, 'Withdrawal request created')
...
export async function listAllWithdrawalRequests() {
  logger.info('Listing all withdrawal requests (owner)')
...
export async function approveWithdrawal(withdrawalId: string) {
  ...
  const { data: profile, error: profileError } = await supabase
    .from('manager_profiles')
    .select('balance_payable, balance_paid')
  ...
  if (profile.balance_payable < request.amount) {
    logger.warn(
      { managerUserId: request.manager_user_id, requested: request.amount, available: profile.balance_payable },
      'Insufficient balance for withdrawal'
    )
    throw new AppError('Недостаточно средств для вывода', 400)
  }
  ...
  // Обновляем баланс напрямую - НЕ трогаем комиссии
...
export async function rejectWithdrawal(withdrawalId: string, note?: string) {
  ...
  logger.info({ withdrawalId }, 'Withdrawal rejected')
}
